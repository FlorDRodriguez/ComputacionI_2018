...
int main (int argc, char* const argv[]) {
    ...
    con = driver->connect("tcp://127.0.0.1:3306", "root", "kaluli32");
	con->setSchema("kiosco");
    ...
    if (Post.find("nombre")==Post.end() )
    {
        ...
        cout<<"<div>"<<endl;
        while (res->next()) {
			cout << res->getString("cli_nombre") << "<br>" << endl;
		}
        cout<<"</div>"<<endl;
	...
	if (Post.find("nombre")!=Post.end() && Post.find("apellido")!=Post.end()) {
		...
		string stringSQL;
		stringSQL = "INSERT INTO cliente (cli_nombre) VALUES ('"+Post["nombre"]+"')";
		pstmt = con->prepareStatement(stringSQL);
		pstmt->executeUpdate();
		delete pstmt;
